...
// 方法讀入，放入List型別的實例變數 url 內。//urlPatterns = { "/*" }, 
@WebFilter( urlPatterns = { "/*" },
		initParams = { 
//				@WebInitParam(name = "mustLogin1", value = "/restaurant/*"), 
				@WebInitParam(name = "mustLogin2", value = "/accommodation/edit/*"), 
//				@WebInitParam(name = "mustLogin3", value = "/Attraction/*"),
				@WebInitParam(name = "mustLogin4", value = "/social/Match.jsp"),
//				@WebInitParam(name = "mustLogin5", value = "/social/*")
		})
public class LoginCheckingFilter implements Filter {
	...
	public void doFilter(ServletRequest request, ServletResponse response,
		...
		if (request instanceof HttpServletRequest
			...
			System.out.println("紀錄是"+ req.getRequestURI());
		...
		for (String sURL : url) {
			if (sURL.endsWith("*")) {
				...
				if (servletPath.startsWith(sURL)) {
					...
					break;
					//for Match.jsp*
				}else if(requestURI.startsWith(sURL)) {
						login = true;
						break;
				}
			} else {
