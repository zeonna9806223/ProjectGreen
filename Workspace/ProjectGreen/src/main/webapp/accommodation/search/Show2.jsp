		...
		<div class="container">
			<div class="row">
						...
						<c:forEach var="i" items="${hInfo}">
							<table style="width:500px"">
								<tr>
									...
									<td>
											<button type="button" id="booking" class="button hide" style="float:right" 
											onclick="num();window.open('${hInfo.h1.web}');">訂房</button>
									</td>
								</tr>
							</table>
							<table>
								<iframe width="400" height="400" frameborder="0" scrolling="no"
									...
									src=https://maps.google.com.tw/maps?f=q&hl=zh-TW&geocode=&q=${i.value.addr}&z=16&output=embed&t=></iframe>
								</tr>
								<tr>
									<td><b><p>地址:</p></b></td>
									<td><p>${i.value.addr}</p></td>
								</tr>
								<tr>
									<td><b><p>電話:</p></b></td>
									<td><p>${i.value.tel}</p></td>
								</tr>
								<c:if test="${i.value.email != ''}">
									<tr>
										<td><b><p>電郵:</p></b></td>
										<td><p>${i.value.email}</p></td>
									</tr>
								</c:if>
								<c:if test="${i.value.web != ''}">
									<script>
										$("#booking").removeClass("hide");
									</script>
									<tr>
										<td><b><p>官網:</p></b></td>
										<td><a onclick="window.open('${i.value.web}')">
												<p id="web">${i.value.web}</p>
										</a></td>
									</tr>
								</c:if>
								<tr>
									<td><b><p>最低價:</p></b></td>
									<td><p>${i.value.price}</p></td>
								</tr>
							</table>
						</c:forEach>
					</div>
				...
				<div class="col-md-4">
					<c:forEach var="i" items="${hInfo}">
						<table>
							<c:if test="${i.value.picture1 != ''}">
								<div>

									<img style="width: 62px; height: 83px; display: block"
										class="smimg" src="${i.value.picture1}"
										onclick="showMaxImg(this)">
									<p>${i.value.pInfo1}</p>

								</div>
							</c:if>
							<c:if test="${i.value.picture2 != ''}">
								<div>

									<img style="width: 62px; height: 83px; display: block"
										class="smimg" src="${i.value.picture2}"
										onclick="showMaxImg(this)">
									<p>${i.value.pInfo2}</p>

								</div>
							</c:if>
							<c:if test="${i.value.picture3 != ''}">
								<div>

									<img style="width: 62px; height: 83px; display: block"
										class="smimg" src="${i.value.picture3}"
										onclick="showMaxImg(this)">
									<p>${i.value.pInfo3}</p>
